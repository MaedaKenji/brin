...
# POI CSV used for indoor/outdoor decision (default, can be overridden)
POI_CSV_PATH = os.path.join(BASE_DIR, "poi_buildings_english.csv")

# Dynamic POI seed default path
POI_SEED_PATH = os.path.join(BASE_DIR, "poi_seed.csv")
...
def analyze_image(
    ...
    Args:
        poi_csv_path: Optional path to a custom POI CSV for GPS queries.
                      If None, uses the default poi_buildings_english.csv.
    """
    ...
    gps_section = None
    if lat is not None and lng is not None:
        poi_result = gps_query(lat, lng, gps_radius, csv_path=poi_csv_path)
        gps_section = {
            "source": gps_source,
            "lat": lat,
            "lng": lng,
...
def _load_pois(csv_path: Optional[str] = None) -> list[dict]:
    """Load building POIs from the CSV file (cached after first call)."""
    global _poi_cache

    # If a custom path is given, load directly without caching in the module-level cache
    if csv_path and csv_path != POI_CSV_PATH:
        pois: list[dict] = []
        try:
...
def determine_environment(
    ...
    Strategy:
        Find the nearest building POI from the POI CSV.
...
def resolve_model(
    ...
    if mode == ModelMode.INDOOR:
        ...
        if lat is not None and lng is not None:
            env_info = determine_environment(lat, lng, indoor_radius_m, poi_csv_path)
...
class FetchPoisRequest(BaseModel):
    """Request body for the /api/fetch-pois endpoint."""
    lat: float = Field(..., description="Center latitude (decimal degrees)")
    lng: float = Field(..., description="Center longitude (decimal degrees)")
    radius_m: float = Field(1000, description="Search radius in metres (default 1000)", ge=50, le=50000)
    output_file: Optional[str] = Field(None, description="Custom output CSV filename (default: poi_seed.csv)")


class EnrichRequest(BaseModel):
    """Request body for the /api/enrich endpoint."""
    input_csv: str = Field("poi_seed.csv", description="Input POI CSV file path")
    output_csv: str = Field("poi_enriched.csv", description="Output enriched CSV file path")
    reviews_json: str = Field("reviews.json", description="Output reviews JSON file path")
    max_reviews: int = Field(20, description="Maximum reviews to extract per POI", ge=1, le=100)


# ── FastAPI app ──────────────────────────────────────────────────────────────────

app = FastAPI(
    title="Surrounding Awareness API (v2 — Dynamic Location)",
    description=(
        "YOLO object/action detection + dynamic GPS POI awareness. "
        "Supports any location worldwide with auto-detection and manual input."
    ),
    version="2.0.0",
)
...
def startup():
    # Eagerly pre-load the outdoor model and default POI data at startup
...
async def api_analyze(
    ...
    poi_csv: Optional[str] = Form(None),
):
    """
    ...
    - **poi_csv**: Optional path to a custom POI CSV file. If not provided,
      uses the default poi_buildings_english.csv.
    """
    ...
    # Resolve POI CSV path
    poi_csv_path = None
    if poi_csv:
        poi_csv_path = os.path.join(BASE_DIR, poi_csv) if not os.path.isabs(poi_csv) else poi_csv
...
# ── New dynamic location endpoints ───────────────────────────────────────────────


@app.get("/api/location/auto")
def api_auto_location():
    """
    Auto-detect the server's current location via IP geolocation.
...
@app.post("/api/fetch-pois")
def api_fetch_pois(req: FetchPoisRequest):
    """
    Dynamically fetch POIs from OpenStreetMap for any location.

    This is the "Fase 1: Geolocation Seeding" step.
    ...
    try:
        pois = fetch_pois(lat=req.lat, lng=req.lng, radius_m=req.radius_m)
        save_csv(pois, output_file)

        # Invalidate GPS cache so the new POI data is picked up
        gps_invalidate_cache()
        ...
        return JSONResponse(content={
            ...
            "pois": pois[:50],  # Return first 50 in the response
            "truncated": len(pois) > 50,
        })
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"POI fetch failed: {e}")


@app.post("/api/enrich")
def api_enrich(req: EnrichRequest):
    """
    Enrich POIs with Google Maps review data using Selenium.

    This is the "Fase 2: Enrichment" step. It is optional and may fail
    if Google Maps blocks the scraping.
    """
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "enrich_reviews module not found or Selenium not installed. "
                "Install with: pip install selenium webdriver-manager"
            ),
        )

    input_path = os.path.join(BASE_DIR, req.input_csv) if not os.path.isabs(req.input_csv) else req.input_csv
    output_path = os.path.join(BASE_DIR, req.output_csv) if not os.path.isabs(req.output_csv) else req.output_csv
    reviews_path = os.path.join(BASE_DIR, req.reviews_json) if not os.path.isabs(req.reviews_json) else req.reviews_json

    if not os.path.exists(input_path):
        raise HTTPException(
            status_code=404,
            detail=f"Input CSV '{req.input_csv}' not found. Run /api/fetch-pois first.",
        )
    ...
    try:
        ...
        return JSONResponse(content={
            ...
            "reviews_json": os.path.basename(reviews_path),
        })
    except Exception as e:
