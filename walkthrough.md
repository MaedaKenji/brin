...
10. [Optional: Google Maps Review Enrichment](#10-optional-google-maps-review-enrichment)
...
| File | Purpose |
|---|---|
| `poi_buildings_english.csv` | Default POI dataset (NCU campus, Taiwan) — used for indoor/outdoor detection |
...
The server includes a **built-in web dashboard** at `http://localhost:8000/`. It provides:

- **Image/Video upload** with real-time YOLO analysis
- **Live webcam streaming** with configurable inference interval
- **GPS configuration** for POI-aware detection
- **Model selection** (Auto, Indoor, Outdoor, Base)
- **Dark/Light theme toggle** with system preference detection

> [!NOTE]
> The frontend is a lightweight set of static files (~20 KB total) served directly
> by FastAPI. It uses in-browser JSX transpilation via CDN-loaded React and Babel —
> **no Node.js, npm, or build step required**. It has negligible impact on RAM,
> CPU, and GPU usage.
...
1. **Create a New Request**: Set method to **POST**, enter URL `http://localhost:8000/api/analyze`
2. **Set Body Type**: Go to **Body** tab → select **form-data**
3. **Configure Parameters**:

    | Key | Type | Value | Description |
    |---|---|---|---|
    | `image` | **File** | `[Select your image]` | Click "Select Files" to upload a JPEG or PNG |
    | `lat` | Text | `24.968` | (Optional) Latitude |
    | `lng` | Text | `121.191` | (Optional) Longitude |
    | `gps_radius` | Text | `500` | (Optional) POI search radius in meters |

    > [!TIP]
    > To change a key's type to **File** in Postman, hover over the key row, click
    > the dropdown on the right of the "Key" field, and select **File**.

4. **Send the Request**: Click **Send**

### Import Collection (Fast Way)

Save the JSON below as `surrounding_awareness.postman_collection.json` and import it into Postman:
...
## 10. Optional: Google Maps Review Enrichment
...
| Blank page at `http://localhost:8000/` | Open browser DevTools (F12) → Console for errors. Ensure CDN scripts (`unpkg.com`) are reachable |
